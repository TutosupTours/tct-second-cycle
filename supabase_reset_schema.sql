-- Script SQL pour corriger complètement le schéma Supabase ECOS
-- À exécuter dans le SQL Editor de Supabase

-- Étape 1: Supprimer toutes les tables existantes (dans l'ordre inverse des dépendances)
DROP TABLE IF EXISTS results CASCADE;
DROP TABLE IF EXISTS evaluations CASCADE;
DROP TABLE IF EXISTS evaluation_grids CASCADE;
DROP TABLE IF EXISTS candidate_passes CASCADE;
...
DROP TABLE IF EXISTS br_members CASCADE;
DROP TABLE IF EXISTS faculty CASCADE;
DROP TABLE IF EXISTS examiners CASCADE;
...
-- Étape 2: Recréer le schéma complet depuis la migration
...
SELECT '✅ Schéma ECOS recréé avec succès ! Toutes les tables, index, politiques RLS et triggers sont en place.' as status;